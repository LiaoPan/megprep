...
def main():
    ...
    if os.path.exists(artifact_ic_output_file):
        ...
        scores_dict = {'ecg': None,'ecg_indices': None,'eog': None,'eog_indices': None}
        try:
            # check eog flat.
            if config["find_bads_eog"]['ch_name'] is not None:
                eog_signal = raw.copy().pick(config["find_bads_eog"]['ch_name']).get_data()
            ...
            print(f"EOG indices:{eog_indices}_eog_scores:{eog_scores}")
            ...
            scores_dict['eog'] = eog_scores[eog_indices]
            scores_dict['eog_indices'] = eog_indices
