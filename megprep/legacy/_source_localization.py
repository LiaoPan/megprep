...
def visualize_covariance_and_spectra(data_cov, raw_data, subj_src_path):
    ...
    fig_cov, fig_spectra = mne.viz.plot_cov(data_cov, raw_data.info)
...
def compute_dSPM(evoked, fwd, noise_cov, subj_src_path, subject_id, subjects_dir, epoch_label, spacing, config, visualize):
    """
    ...
    print("**config.get('dSPM')['inverse_operator']:",config.get('dSPM')['inverse_operator'])
    inverse_operator = make_inverse_operator(info=evoked.info, forward=fwd, noise_cov=noise_cov,
                                             **config.get('dSPM')['inverse_operator'])
    print("**config.get('dSPM')['apply_inverse']：",config.get('dSPM')['apply_inverse'])
    stc = apply_inverse(evoked, inverse_operator, **config.get('dSPM')['apply_inverse'])
    stc.save(stc_file, overwrite=True)

    if visualize:
        visualize_source_estimate(stc, subject_id, subjects_dir, subj_src_path, epoch_label, "dSPM", spacing, block=False)
...
def process_subject(epoch_file, fs_subjects_dir, noise_cov_path, fwd_dir, output_dir, config, visualize):
    ...
    try:
        ...
        if 'dSPM' in config.get("source_methods"):
            # Compute dSPM
            compute_dSPM(evoked, fwd, noise_cov, output_dir, subject_id, fs_subjects_dir, epoch_label, spacing, config,visualize)
    ...
    try:
        ...
        if 'dSPM' in config.get("source_methods"):
            ...
            stc = apply_inverse_raw(raw, inverse_operator,
                                    **config.get('dSPM')['apply_inverse'])
            stc.save(os.path.join(output_dir, f"{epoch_label}_raw_dSPM-{spacing}"), overwrite=True)
            if visualize:
                visualize_source_estimate(stc, subject_id, fs_subjects_dir, output_dir, epoch_label, "dSPM", spacing, block=False)
